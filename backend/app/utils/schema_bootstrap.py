...
async def ensure_runtime_schema(conn: AsyncConnection) -> None:
    ...
    await conn.execute(text("ALTER TABLE system_settings ADD COLUMN IF NOT EXISTS auto_send_invoice_on_approval BOOLEAN NOT NULL DEFAULT true"))
    ...
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_customers_phone_suffix ON customers (right(regexp_replace(coalesce(phone, ''), '\\D', '', 'g'), 8))"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_hydrometers_customer_active ON hydrometers (customer_id, is_active)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_hydrometers_active_code ON hydrometers (is_active, code)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_readings_status_created_at ON readings (status, created_at DESC)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_readings_hydrometer_created_at ON readings (hydrometer_id, created_at DESC)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_invoices_status_due_date ON invoices (status, due_date DESC)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_invoices_customer_status_due ON invoices (customer_id, status, due_date DESC)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_invoices_customer_paid_date ON invoices (customer_id, paid_date DESC) WHERE paid_date IS NOT NULL"))
    ...
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_whatsapp_messages_phone_created_at ON whatsapp_messages (phone, created_at DESC)"))
    await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_whatsapp_messages_external_message_id ON whatsapp_messages (external_message_id)"))
